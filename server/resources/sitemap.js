...
const servioServiceCategories = require('./servioSeoPaths.js').serviceCategories;
const servioServiceAreas = require('./servioSeoPaths.js').serviceAreas;
...
const defaultPublicPaths = {
  ...
  login: { url: '/login' },
  search: { url: '/s' },
  // SERVIO's SEO landing pages (real content per category/area, see
  // src/containers/ServiceCategoryPage and ServiceAreaPage, and
  // server/resources/servioSeoPaths.js for the shared list).
  ...Object.fromEntries(
    servioServiceCategories.map(slug => [`serviceCategory_${slug}`, { url: `/services/${slug}` }])
  ),
  ...Object.fromEntries(
    servioServiceAreas.map(slug => [`serviceArea_${slug}`, { url: `/location/${slug}` }])
  ),
};
