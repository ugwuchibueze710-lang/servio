...
export const TransactionPageComponent = props => {
  ...
  // A downloadable invoice/receipt only makes sense once the transaction has actually been paid -
  // payinTotal is set by Sharetribe once the Stripe payment succeeds, so this is a real state check,
  // not a guess. Either party (customer or provider) can generate their own copy - see
  // src/components/InvoiceDownloadButton and server/api/invoice-pdf.js.
  const hasBeenPaid = !!transaction?.attributes?.payinTotal;
