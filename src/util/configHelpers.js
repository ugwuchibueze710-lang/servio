...
const getSortOptionsFromListingFields = listingFields => {
  ...
  // listingFields can be undefined/empty if hosted listing-fields config hasn't been set up yet
  // (e.g. a brand new marketplace before Console has been fully configured) or is temporarily
  // unreachable - don't crash server-side rendering over it.
  const fields = Array.isArray(listingFields) ? listingFields : [];
...
const mergeListingConfig = (hostedConfig, defaultConfigs, categoriesInUse) => {
  ...
  // SERVIO ships 2 listing types (service / job-request) defined locally in
  // src/config/configListing.js. Passing `true` here means they're merged in during local
  // development (NODE_ENV=development) so the app is testable before Console is configured.
  // IMPORTANT: mergeDefaultTypesAndFieldsForDebugging() still forces this off in production builds
  // (NODE_ENV=production) - that's intentional, matching Sharetribe's guidance not to rely on local
  // debug config in production. Before launch, create matching listing types/fields in Sharetribe
  // Console (or push them with Sharetribe CLI) using the same 'listingType'/'key' values - see
  // sharetribe-setup/README.md. Once Console has them, those hosted values take precedence anyway.
