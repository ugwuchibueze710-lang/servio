...
// Ride-specific privileged endpoints. See server/api/ride-initiate-privileged.js
// and server/api/ride-transition-privileged.js for why Ride doesn't reuse
// initiatePrivileged/transitionPrivileged above: ride dispatch needs the
// server to choose the listing (driver) before a transaction can even be
// created, which the generic endpoints don't support.
export const rideInitiatePrivileged = body => {
  return post('/api/ride/initiate-privileged', body);
};

export const rideTransitionPrivileged = body => {
...
export const INVOICE_PDF_TEMPLATES = [
  { id: 'professional-modern', name: 'Professional' },
  { id: 'minimalist-clean', name: 'Minimalist' },
  { id: 'nova', name: 'Nova' },
  { id: 'slate', name: 'Slate' },
];

// Build the URL for downloading a completed transaction's invoice/receipt PDF.
//
// This is a same-origin, cookie-authenticated GET endpoint (see `server/api/invoice-pdf.js`), so
// unlike the other helpers here it isn't fetched with `post()` - a plain link/window.open lets the
// browser handle the file download natively via the endpoint's Content-Disposition header.
export const invoicePdfUrl = (transactionId, templateId) => {
  const base = `/api/invoice-pdf/${transactionId}`;
