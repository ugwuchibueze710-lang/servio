...
export const listingFields = [
  {
    ...
    saveConfig: {
      label: 'Years in business',
      placeholderMessage: 'e.g. 5',
      isRequired: false,
    },
  },
  {
    key: 'isLicensedInsured',
    scope: 'public',
    schemaType: 'boolean',
    filterConfig: {
      indexForSearch: false,
    ...
    saveConfig: {
      label: 'I am licensed and insured for this work',
      isRequired: false,
    },
  },
  {
    ...
    saveConfig: {
      label: 'How far will you travel for a job? (miles)',
  ...
  // Ride driver vehicle/eligibility fields - see RIDE_INTEGRATION_REPORT.md and
  // src/ride/rideDataSchema.js (the single source of truth these mirror). `isOnline` needs
  // `indexForSearch: true` because server/api-util/rideDispatch.js filters candidate drivers with
  // `pub_isOnline=true` - without the search index in Console, that query silently returns nothing.
  {
    key: 'vehicleMake',
    scope: 'public',
    schemaType: 'text',
    listingTypeConfig: { limitToListingTypeIds: true, listingTypeIds: ['ride-driver'] },
    showConfig: { label: 'Vehicle make' },
    saveConfig: { label: 'Vehicle make', placeholderMessage: 'e.g. Toyota', isRequired: true },
  },
  {
    key: 'vehicleModel',
    scope: 'public',
    schemaType: 'text',
    listingTypeConfig: { limitToListingTypeIds: true, listingTypeIds: ['ride-driver'] },
    showConfig: { label: 'Vehicle model' },
    saveConfig: { label: 'Vehicle model', placeholderMessage: 'e.g. Camry', isRequired: true },
  },
  {
    key: 'vehicleColor',
    scope: 'public',
    schemaType: 'text',
    listingTypeConfig: { limitToListingTypeIds: true, listingTypeIds: ['ride-driver'] },
    showConfig: { label: 'Vehicle color' },
    saveConfig: { label: 'Vehicle color', placeholderMessage: 'e.g. Silver', isRequired: true },
  },
  {
    key: 'vehicleYear',
    scope: 'public',
    schemaType: 'long',
    numberConfig: { minimum: 1990, maximum: 2100 },
    listingTypeConfig: { limitToListingTypeIds: true, listingTypeIds: ['ride-driver'] },
    showConfig: { label: 'Vehicle year' },
    saveConfig: { label: 'Vehicle year', isRequired: true },
  },
  {
    key: 'licensePlate',
    scope: 'public',
    schemaType: 'text',
    listingTypeConfig: { limitToListingTypeIds: true, listingTypeIds: ['ride-driver'] },
    showConfig: { label: 'License plate' },
    saveConfig: { label: 'License plate', isRequired: true },
  },
  {
    key: 'rideType',
    scope: 'public',
    ...
    listingTypeConfig: { limitToListingTypeIds: true, listingTypeIds: ['ride-driver'] },
    filterConfig: { indexForSearch: true, group: 'primary', label: 'Ride type' },
    showConfig: { label: 'Ride type' },
    saveConfig: { label: 'Ride type', isRequired: true },
  },
  {
    key: 'isOnline',
    scope: 'public',
    schemaType: 'boolean',
    listingTypeConfig: { limitToListingTypeIds: true, listingTypeIds: ['ride-driver'] },
    filterConfig: { indexForSearch: true, group: 'primary', label: 'Online now' },
    showConfig: { label: 'Online now', isDetail: false },
    saveConfig: { label: 'Online now', isRequired: false },
  },
];
...
export const listingTypes = [
  // SERVIO's two core listing types both use Sharetribe's built-in `default-negotiation` transaction
  // process (quote/offer + Stripe payment + delivery + two-sided reviews). No custom transaction
  // system is invented here - this is the process Sharetribe Console must also have enabled for the
  // marketplace environment (Build > Transaction process). See sharetribe-setup/README.md.
  //
  // 1) Providers publish a "Service" listing with their own pricing. Customers browsing it can
  //    "Request a quote" (unitType: 'offer' => provider created the listing => forward mode).
  {
    listingType: 'service',
    label: 'Service (set your own pricing)',
    transactionType: {
  ...
  // 2) Customers publish a "Job request" describing what they need done, with photos and a budget.
  //    Providers browsing it can "Make an offer" (a quote) or "Inquire" first
  //    (unitType: 'request' => customer created the listing => reverse mode).
  {
    listingType: 'job-request',
    label: 'Job request (post what you need done)',
    transactionType: {
      process: 'default-negotiation',
      alias: 'default-negotiation/release-1',
      unitType: 'request',
    ...
    defaultListingFields: {
      location: true,
      // The customer posting the job is the one who gets paid nothing here - they pay the provider
      // once they accept a quote - so they don't need payout details to publish a request.
      payoutDetails: false,
    },
    transactionFields: [
      {
      ...
      },
      {
        ...
        schemaType: 'enum',
        enumOptions: [
          { option: 'asap', label: 'As soon as possible' },
          { option: 'this-week', label: 'This week' },
          { option: 'this-month', label: 'This month' },
          { option: 'flexible', label: "I'm flexible" },
        ],
      },
    ],
    messagingOptions: {
  ...
  // 3) Ride: drivers publish a "Ride driver" listing (vehicle + license plate details) using the
  //    new custom `ride` transaction process (ext/transaction-processes/ride/process.edn) instead of
  //    default-negotiation - see RIDE_INTEGRATION_REPORT.md and sharetribe-setup/ride-deployment.md.
  //    `unitType: 'item'` is used here (as the commented-out default-purchase example below does)
  //    because Ride computes its own line items entirely in server/api-util/ridePricing.js rather
  //    than using Sharetribe's built-in day/night/hour/item quantity helpers - the unitType value
  //    itself doesn't drive Ride's pricing logic, it's just the required config shape.
  {
    listingType: 'ride-driver',
    label: 'Ride driver',
    transactionType: {
      process: 'ride',
      alias: 'ride/release-1',
      unitType: 'item',
    },
    defaultListingFields: {
      location: false,
