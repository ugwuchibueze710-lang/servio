...
const ServiceAreaPage = loadable(() => import(/* webpackChunkName: "ServiceAreaPage" */ '../containers/ServiceAreaPage/ServiceAreaPage'));
...
const routeConfiguration = (layoutConfig, accessControlConfig) => {
  ...
  return [
    ...
    // SEO landing pages: real, unique content per category/area that links into genuine search
    // results (see src/containers/ServiceCategoryPage and ServiceAreaPage). Content is data-driven
    // from src/config/configServiceCategories.js and configServiceAreas.js, so admins can add a new
    // category or launch city without touching routing or component code.
    {
    ...
    // Ride - see RIDE_INTEGRATION_REPORT.md. Both routes require auth since
    // driving/requesting a ride both need a real Servio identity (spec
    // section 3) - there is no separate rider/driver login.
    {
      path: '/ride',
      name: 'RidePage',
      auth: true,
